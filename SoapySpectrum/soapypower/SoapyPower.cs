...
namespace SoapySpectrum.soapypower
{
    internal class SoapyPower
    {
        ...
        public static void updateFrequency() => sendPipeCommand($"change_frequency@{Configuration.config["freqStart"]}@{Configuration.config["freqStop"]}");
        public static void updateGain(string gainName, float value) => sendPipeCommand($"change_gain@{gainName}@{value}");
        ...
        public static void beginStream()
        {
            ...
            soapyProcThread = new Thread(() =>
            {
                ...
                soapyPowerPROC.StartInfo.Arguments = $"{Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "soapypower\\__main__.py")} -r 52e6 -b {Configuration.config["FFTSize"]} -f {Configuration.config["freqStart"]}:{Configuration.config["freqStop"]} --pow2 -d driver={Configuration.config["driver"]},master_clock_rate=52e6 -c {Configuration.config["additional"]} -C 0 -q --reset-stream --crop 30 -n {Configuration.config["weleching"]} -D constant --fft-window hamming";
