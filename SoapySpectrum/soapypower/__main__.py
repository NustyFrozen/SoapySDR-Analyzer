...
import os, sys, logging, argparse, re, shutil, textwrap,threading
import copy
import simplesoapy
...
def setup_argument_parser():
    ...
    main_title.add_argument('--RL', action='store_true',
                            help='perform sweep with siggen on the specified freq range')
...
def main():
    ...
    threading.Thread(target=pipe.startPipe, args=(sdr,args)).start()
    #if args.RL:
        #threading.Thread(target=siggen.sweep_siggen, args=(sdr.device.device,31250,0.7,920,5e6,0,30,None,None,None,args.freq[0], args.freq[1])).start()
    ...
    sdr.sweep(
            ...
            max_threads=args.max_threads, max_queue_size=args.max_queue_size,RL=args.RL
        )
