...
public static class tab_Cal
{
    ...
    public static Dictionary<string, Tuple<bool, float, int>> s_calGain = new();
    ...
    public static bool s_isCalibrating;
    public static int s_selectedCal = -1;
    public static List<CalibrationPoint> s_currentCalData = new();

    public static void loadCalibration(string name)
    {
        s_currentCalData =
            JsonConvert.DeserializeObject<List<CalibrationPoint>>(
                File.ReadAllText(Path.Combine(Configuration.calibrationPath, $"{name}.cal")));
    }

    public static void renderCalibration()
    {
        Theme.Text("Select Calibration");
        if (Theme.glowingCombo("sample_rate_Tab", ref s_selectedCal, s_AvailableCal, Theme.inputTheme))
        ...
        for (var i = 0; i < tab_Device.s_deviceGains.Count(); i++)
        {
            var currentGain = tab_Device.s_deviceGains[i];
            if (s_calGain.Keys.All(x => x != currentGain.Item1))
            ...
            if (ImGui.Checkbox($"Calibrate {currentGain.Item1}", ref isCalGain))
                s_calGain[currentGain.Item1] = new Tuple<bool, float, int>(isCalGain,
                    s_calGain[currentGain.Item1].Item2, s_calGain[currentGain.Item1].Item3);
            if (isCalGain)
            {
                ...
                if (ImGui.SliderFloat($"Step accuracy for {currentGain.Item1} (%)", ref step, 1, 100))
                    ...
                    s_calGain[currentGain.Item1] =
                        new Tuple<bool, float, int>(isCalGain, step, s_calGain[currentGain.Item1].Item3);
    ...
    public static void beginCalibration()
    {
        ...
        var calibrationResults = new List<CalibrationPoint>();
        ...
        new Thread(() =>
        {
            ...
            while (true)
            {
                ...
                if (freqStart >= freqStop ||
                    !tab_Device.s_deviceFrequencyRange[0].ToList()
                        .Exists(x => x.Minimum <= freqStart && x.Maximum >= freqStop)
                    || currentFreq > s_freqStop)
                {
                ...
                s_calInfo =
                    $"Please transmit {s_inputDB} at {currentFreq} and then press enter\n when you see the signal and the FFT is stable\n" +
                    $"\nFound\nFreq: {frequencyFound}\ndB:{currentdB}";
                ...
                {
                    var x = new CalibrationPoint { frequency = currentFreq, results = s_inputDB - currentdB };
                    ...
                    foreach (var keyvalue in s_calGain)
                    {
                        ...
                        for (double i = 0; i < 100; i += keyvalue.Value.Item2)
                        {
                            ...
                            var elementChange =
                                tab_Trace.findMaxHoldRange(tab_Trace.s_traces[0].plot, (float)freqStart,
                                    (float)freqStop).Value + currentdB;
                    ...
                    while (Imports.GetAsyncKeyState(Keys.Enter)) Thread.Sleep(50);
                ...
                if (Imports.GetAsyncKeyState(Keys.End)) break;
            }
    ...
    {
        var data = JsonConvert.SerializeObject(calibrationResults);
        data = data.Replace(",", "\n,").Replace("{", "\n{").Replace("}", "\n}");
