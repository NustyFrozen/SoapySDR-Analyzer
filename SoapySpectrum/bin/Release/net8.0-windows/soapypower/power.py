...
import sys, time, datetime, math, logging, signal,threading
...
class SoapyPower:
    ...
    def __init__(self, soapy_args='', sample_rate=2.00e6, bandwidth=0, corr=0, gain=20.7,
        ...
        self._psd = None
        self._writer = None
        self._min_freq = 930000000.0
        self._max_freq = 960000000.0
    ...
    def setup(self, bins, repeats, base_buffer_size=0, max_buffer_size=0, fft_window='hann',
              ...
              lnb_lo=0, tune_delay=0, reset_stream=False, max_threads=0, max_queue_size=0,RL=False):
        ...
        if RL:
            ...
            wave_freq = rate / 10
            ampl = 0.7
            stream_mtu = self.device.device.getStreamMTU(self.tx_stream)
            self.samps_chan = numpy.array([ampl]*stream_mtu, numpy.complex64)
            phase_acc = 0
    ...
    def siggen(self,freq):
            ...
            self.device.device.activateStream(self.tx_stream)
            self.device.device.setGain(SoapySDR.SOAPY_SDR_TX, 0, 50)
            self.device.device.writeStream(self.tx_stream, [self.samps_chan], self.samps_chan.size, timeoutUs=1000000)
    ...
    def sweep(self, min_freq, max_freq, bins, repeats, runs=0, time_limit=0, overlap=0,
              ...
              tune_delay=0, reset_stream=False, base_buffer_size=0, max_buffer_size=0, max_threads=0, max_queue_size=0,RL=False):
        ...
        self.setup(
            ...
            reset_stream=reset_stream, max_threads=max_threads, max_queue_size=max_queue_size,RL=RL
        )
        self._min_freq = min_freq
        self._max_freq = max_freq
        freqcounter = self._min_freq
        try:
            self.freq_list = self.freq_plan(min_freq - lnb_lo, max_freq - lnb_lo, bins, overlap)
            t_start = time.time()
            run = 0
            while not _shutdown and (runs == 0 or run < runs):
                ...
                for freq in self.freq_list:
                    if RL:
                        self.siggen(freq=int(freqcounter))
                        if(freqcounter >= self._max_freq or freqcounter < self._min_freq): #out of frequency bounds
                            freqcounter = self._min_freq
                        else:
                            freqcounter += 1000000.0
